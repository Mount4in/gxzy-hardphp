...
class View
{
    protected $left_delimiter, $right_delimiter, $template_dir, $compile_dir;
    protected $template_vals = array();
    ...
    public function compile($tempalte_name)
    {
        ...
        if (!file_put_contents($tmp_file, $template_data)) {
            err('Err: File \\\\\"' . $tmp_file . '\\\\\" can not be generated.');
        }
        $success = @rename($tmp_file, $complied_file);
        if (!$success) {
            if (is_file($complied_file)) {
                @unlink($complied_file);
            }
            $success = @rename($tmp_file, $complied_file);
        }
        if (!$success) {
            err('Err: File \\\\\"' . $complied_file . '\\\\\" can not be generated.');
        }
        return $complied_file;
    }
